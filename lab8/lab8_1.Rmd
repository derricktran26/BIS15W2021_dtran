...
heartrate
```
#so the columns here are tricky in that it counts as data instead of name, so the first 2 observations is not met, 
...
```
you can now see that drug has its own column and the others have their own column. normal data entry no one would actually do this, but in data analysis its imporant to do this
## Practice
...
```{r}
 relig_income
```
cause the columns for income are both variable and observations?
...
billboard2 <- 
  ...
  pivot_longer(wk1:wk76, # a range of columns
               names_to = "week, your moving those columns to this",
               values_to = "rank",
               values_drop_na = TRUE #this will drop the NA's if you notice theres a lot
               )
billboard2
```
...
billboard3 <- 
  billboard %>% 
  pivot_longer(-c(artist, track, date.entered), #specific columns keep those columns 
               names_to = "week",
               values_to = "rank",
               values_drop_na = TRUE
               )
...
billboard %>% 
   ...
   values_drop_na = TRUE)
```
you got rid of the wk in the week column
## Practice  
...
plant_data%>%
  pivot_longer(
    cols=starts_with("day"), #like how we use the range 
    names_to = "day",
    names_prefix = "day",  #removes the prefix in the data
    values_to = "measurement")%>%
  select(genotype,day,measurement)
  
```
#renames the later column that shows measurement,
...
youre going to move the names to experiment and replicate, youre pulling the data columns apart above, but you will have to tell R that you want to pull the name apart at the underscore 
```{r}
qpcr_untidy %>% 
  pivot_longer(
    exp1_rep1:exp3_rep3,
