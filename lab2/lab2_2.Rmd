...
```
## this way you can find exactly where it is, if you do what you did for days of the week it'll just spit it back out and give you nothing. Also note that you have to use two equal signs because if you dont you end up making a new vector
...
#you can set up a data matrix by using rows, I think you play with the nrow function. if you use nrow=4 itll look funkyand if you do a nrow16 you will just have 16 rows
#so if your data points are in 8 pairs, you will want to do nrow 8 because it will add things from left to right
#what does byrow do? he said to play around with it
```{r}
...
```{r}
#you are trying to find the average in the HW so you want to make an object that will allow you to do that 
...
all_harry_potter_matrix #cbind means column bind, you can bind it to the matrix. you can also do this for rows too and if you want to add a new data point use r or c bind to put it into the matrix
```
...
```{r}
sum(box_office)
#make a new object total earnings 
```{r}
...
```{r}
#here the format[row,column] in selecting specific parts of a data matrix
