...
#you can use this to find a column that matches your string, like in this case has an a and er
```
...
#note that anything with dbl is considered a numeric so select if can select a certain class of data
#if not you a ~
```
...
```{r}
summary(mammals)
#i think summary is great if the data is fitted for R the issue in this data set is that the scientist recorded NA values as -999 and R will interpret it as is. that is annoying. and you can see when you run summary 
```
...
rename comand and select the data frame. then write new name and set equal to desired new name with parenthesis 
fun fact if you highlight a line of text and tap parenthesis it does it for you
```{r}
...
mammals_new
```
#they made a new data frame with the edited column names to make viewing the data better 
...
we are making a new data frame mass. we will be selecting from the renamed data frame mammals new and will be selecting 3 columns 
you can always change the order in which the columns appear by reordering it in the command select
```{r}
