...
every column is its on vector and filter works with rows
## Load the tidyverse
...
## `filter()`
Unlike `select()`, `filter()` allows us to extract data that meet specific criteria within a variable. Let's say that we are interested only in the fish that occur in lake "AL". We can use `filter()` to extract these observations. botice how the number of items decreased after you selected filter. lakeid is our column here 
```{r}
filter(fish, lakeid == "AL")

```

Similarly, if we are only interested in fish with a length greater than or equal to 350 we can use `filter()` to extract these observations. notice how we are getting lengths of 350 or more now. really know that select is for column and filter is for looking within those columns 
```{r}
...
[operator](https://www.tutorialspoint.com/r/r_operators.htm).    
```{r}
filter(fish, length %in% c(167, 175))
```
#so you are looking for values in column length that are 167 and 175
...
```{r}
#filter(fish, between(scalelength,0.25,8)) wrong function type 
...
Notice that the `|` operator generates a different result.i would treat it like an or instead of an and
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
...
```
USE THE VIEW() TO CHECK IF YOUR WORK STUFFS
## Wrap-up  
