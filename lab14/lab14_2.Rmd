...
## Summaries
Get a quick summary of the data. dashes will usually tell you if theres something missing and they will also show you the distribution of genotypes
```{r}
summary(sug)
```
```{r}
head(sug$pheno)
```
we only care about the first 4 columns really


There are a number of simple functions for pulling out pieces of summary information. from the qtl package
```{r}
...
Individual parts of the above plot may be obtained as follows. you can actually just run individual plots depending on what you need


```{r}
#?plotMissing
plotMissing(sug, main="")
plotMissing(sug, main="", reorder=1)# reorder means we are reordering based on the first value
...
```
Let’s use iplotCorr to plot a heat map of the correlation matrix for the phenotype of all pairs, linked to scatterplots of the phenotypes.when you click on the boxes it will show you the scatter plot for that combination of variables and tell you the correlation too
```{r}
iplotCorr(sug$pheno)
...
#step is the density in which you they do the calculation
```{r}
...
notice that loc 45 has a high association with bp trait
```{r}
summary(out.em)
summary(out.em2)
```
