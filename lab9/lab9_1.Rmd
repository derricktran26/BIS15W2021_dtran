...
The plot below is the same but uses `geom_col()`for plotting bar is for counts or better suited
```{r}
homerange %>% 
  group_by(class) %>% 
...
names(homerange)
```
-10 means bottom 10 and just '10' would ten be top 10
```{r}
homerange %>% 
  filter(class=="mammalia") %>% 
  top_n(-10, log10.mass) %>% 
...
homerange %>% 
  ...
  coord_flip()
```
reorder common.name by log10.mass so it orders your how your x appears by its y axis values
