...
#you can get an excell file and then export it as a csv file and that is the easiet way to set up your data frame. this is the way most data scientist get their data. you must also pay a lot of attention on WHERE you are in your directory. you should make it a habit to do it everytime you start an assignment or start writing code. Do not base your positioning on the file panel its better to check. 

#if you are in the wrong director, go to session, set working directory and choose directory and then click open on the file you want to be in. it will tell you in console where youre at or do setwd(enter location). i think former method is easier tho
```{r}
getwd()
```
...
class(hot_springs$scientist)
```
#the difference between a character and factor. Factors have specific levels 1-7 for ex instead of just having one level
...
```{r}
setwd("~/GitHub/BIS15W2021_dtran/lab3/data")
gaeta_data <- readr::read_csv("Gaeta_etal_CLC_data.csv")
...
glimpse(gaeta_data)
#names(gaeta_data)
#structure(gaeta_data) use these 
```
...
```{r}
table(gaeta_data$scalelength)
#long continuous variable doesn work well, so you want to use this for categorical things. if you use table and something like Lake id
```

We can summarize our data frame with the`summary()` function.  
```{r}
#summary(fish) is what he has but you wrote it as gaeta 
```
