...
```
#remember they got to be equal in length
Here we combine our three vectors to create a data frame with the function `data.frame()`.
```{r}
hbirds <- data.frame(Sex, Length, Weight)
hbirds
```
#data.frame will assemble into a data frame. this already has columns and rows dbl is a numeric
...
hummingbirds_matrix
```
#notice how they look different from a data matrix. The orientaion of matrix is a series of stacked vectors like it goes sex vector weight vector and so on but in data frame they are their own variable. the names of the columns in data frame are important because those are our variables 
...
names(hbirds)
```
#the name command is great because if you forget what is in the data frame and in this case this is for the variable names
#also if you get lost go to the data window and itll show you a excell like spread sheet
...
names(hbirds)
```
#using naming conventions will be more helpful later on. you should rename column names so that they are easier to call 
```{r}
hbirds
```
...
#makes calling out certain columns easier
```{r}
w <- hbirds$weight_oz
mean(w)
```

## Adding Rows we use rbind() we've made a new bird vector make sure for naming you use quotes
...
new_bird
```
#now add this to your data frame, call your frame and row bind it (dataframe, new object)
```{r}
...
# you want to make sure that when you name the file, that you add .csv to the end and we dont want the row names either so make sure to set it to false
#if you want to save it to a particular folder you should give it a path
```{r}
...
```{r}
hspring <- data.frame(name,spring,temp)
hspring <- data.frame(name=name,spring=spring,temp_c = temp)
hspring
```

2. Change the column titled `name` to `scientist`, leave the other column names the same. Print out the data frame names.  
```{r}
hspring <- data.frame(scientist=name,spring=spring,temp_c = temp)
hspring
```
...
hspring <- cbind(hspring,depth_fit)
# you could do it like this but less efficient hspring<- data.frame(name,spring,temp,depth_fit)
hspring
```
...
```{r}
write.csv(hspring, "data/hot_spring_data.csv", row.names = FALSE)
getwd()
```
